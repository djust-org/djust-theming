...
### Added
- **Component Storybook and Marketplace Spec (Phase 9.2 + 9.3)** -- Two new developer tools. **Component Storybook (9.2):** New storybook pages in the gallery module at `theming/gallery/storybook/` (index) and `theming/gallery/storybook/<component>/` (detail). The index page lists all 24 theme components with required/optional context counts and slot counts, linking to detail pages. Each detail page shows: rendered variant examples from the gallery context builders, full context contract table (required and optional variables with types and defaults), accessibility requirements table, available slots list, CSS variables used by the component (extracted from `components.css` and `base.css` rule blocks matching the component's class prefix, plus any inline `var()` references in templates), and raw template source code in a `<pre>` block. New `djust_theming/gallery/storybook.py` module with `get_component_template_source()` (reads default template HTML), `extract_css_variables()` (regex-based `var(--name)` extraction with deduplication), `_get_component_css_variables()` (CSS-class-aware extraction from stylesheets), `build_storybook_index_context()`, `build_storybook_detail_context()`, and `get_component_coverage()` (reusable by both storybook and CLI). Two new URL patterns in `gallery/urls.py`. Two new templates: `storybook_index.html` (card grid layout) and `storybook_detail.html` (full documentation layout). Same access control as gallery (DEBUG=True or is_staff). Returns 404 for unknown component names. **Marketplace Spec (9.3):** New `docs/marketplace-spec.md` documenting the `[marketplace]` section format for `theme.toml` with four fields: `screenshots` (list of image paths), `tags` (freeform categorization tags), `compatibility_range` (PEP 440 version specifier), and `preview_url` (live preview link). `ThemeManifest` dataclass extended with these four fields, parsed from `[marketplace]` section in `from_toml()` and serialized in `to_toml()` (section omitted when all fields are empty). New `marketplace-info` management subcommand (`python manage.py djust_theme marketplace-info <theme-name>`) loads the theme manifest, computes component coverage (percentage of the 24 components that have template overrides vs inheriting defaults), and prints a report showing theme metadata, marketplace fields (tags, compatibility range, preview URL, screenshots), coverage percentage, overridden component list, and inherited component list. Supports `--dir` to override the themes directory. 43 new tests: 31 for storybook (URL resolution, access control, index content, detail content, helper functions, context builders) and 12 for marketplace (manifest field parsing/serialization, component coverage computation, CLI command output).

- **Theme Author Guide and Migration Guide (Phase 9.1 + 9.4)** -- Three new deliverables for theme authors. **Getting Started Guide (9.1):** New `docs/getting-started.md` tutorial "Create your first theme in 10 minutes" with 5 steps: scaffold a theme with `create-theme --preset blue`, override button and card component templates, customize design tokens in `tokens.css`, preview in the gallery and live editor, package with `create-package` and publish to PyPI. Includes code blocks for each step, token reference table, and links to related docs. **Breaking Changes document (9.4):** New `docs/BREAKING_CHANGES.md` establishes the v1.0 contract baseline with a complete summary table of all 24 components listing required elements (with attributes), required context variables, and available slots. Defines the format for tracking future contract changes: version, component, what changed, migration steps. **check-compat command (9.4):** New `python manage.py djust_theme check-compat <theme-name>` subcommand scans a theme's `components/` directory for template overrides and validates each against `COMPONENT_CONTRACTS`. Checks three categories: required HTML elements (tag presence and required attributes like `role="alert"` or `aria-haspopup="true"`), required context variables (detected via `{{ var }}` and `{% tag var %}` patterns), and available slots (reported as info when unused). Reports issues at three severity levels: ERROR (missing required element or context var), WARNING (template with no matching contract), INFO (optional slot not referenced). Supports `--all` flag to check all themes in the directory and `--dir` to override the themes directory. Core logic in new `djust_theming/compat.py` module with `CompatIssue` dataclass and `check_theme_compat()` function. 14 new tests covering no-override baseline, valid overrides, missing elements, missing attributes, missing context vars, unused slots, unknown components, context vars in `{% if %}` tags, multiple required elements, command PASS/ERROR output, missing theme errors, and `--all` flag across multiple themes.

- **Live theme editor and diff view (Phase 8.2 + 8.3)** -- Two new developer tools in the gallery module. **Live Editor (8.2):** New page at `theming/gallery/editor/` with a two-panel layout: left sidebar with token controls, right panel with live component preview. Left panel includes a preset selector dropdown (loads any of the 19 presets), light/dark mode tab switcher, native `<input type="color">` controls for all 31 HSL color tokens (background, foreground, primary, secondary, muted, accent, destructive, success, warning, info, link, code, selection, border, input, ring, and all foreground variants), and a `<input type="range">` slider for `--radius` (0-2rem). All changes update CSS custom properties in real-time via vanilla JS with no server round-trip. Export button POSTs current token values to `theming/gallery/editor/export/` which returns JSON with generated `tokens.css` (CSS custom property declarations for light and dark modes) and `theme.toml` (manifest file), triggering client-side file downloads. Reset button restores the currently selected preset defaults. HSL-to-hex and hex-to-HSL conversion handled in client JS for color picker compatibility. All preset token data serialized as JSON and embedded in the page for instant preset switching. **Diff View (8.3):** New page at `theming/gallery/diff/` with side-by-side preset comparison. Two iframes each load the existing gallery page with `?preset=<name>` query parameter, avoiding code duplication. Top toolbar with left/right preset selector dropdowns (defaults: left=default, right=nord). Responsive layout collapses to stacked on mobile. Floating preset name labels on each panel. **Shared infrastructure:** New `serialize_tokens()`, `serialize_preset()`, and `serialize_all_presets()` helper functions in `gallery/context.py` for converting ThemeTokens/ThemePreset dataclasses to JSON-friendly dicts. Shared `_check_access()` helper in `gallery/views.py` for consistent DEBUG/is_staff access control across all gallery views. Three new URL patterns added to `gallery/urls.py`. Same access control as gallery (DEBUG=True or is_staff). 30 new tests covering URL resolution, access control (editor + diff + export), editor content (color inputs, radius slider, preset selector, export button, preset JSON data, preset param, preview components), export endpoint (GET 405, valid POST returns JSON with tokens_css + theme_toml, CSS custom properties in output, [theme] section in TOML, access control), and diff content (two iframes, preset selectors, default presets, custom preset params).

- **Theme gallery view (Phase 8.1)** -- New developer-facing gallery page at `theming/gallery/` that renders all 24 theme components with every variant, size, and slot combination on a single page. New `djust_theming/gallery/` module with `views.py` (access-gated `gallery_view`), `context.py` (`build_gallery_context()` builds sample data for all components), and `urls.py`. Gallery shows buttons in 5 variants (primary, secondary, destructive, ghost, link) and 3 sizes (sm, md, lg); alerts in 4 variants plus dismissible; badges in 5 variants; tables in 3 variants (default, striped, hover); modals in 3 sizes; toasts in 4 variants; progress bars at 25/50/75/100% plus indeterminate; skeletons in 3 shapes; tooltips in 4 positions; avatars in 3 sizes; and full examples for all form, navigation, and layout components. Includes a preset switcher `<select>` at top (`?preset=<name>` query parameter) so developers can test every component across all 19 color presets. Access control: always available when `DEBUG=True`, otherwise requires `request.user.is_staff` (returns 403 for unauthorized users). Gallery template at `djust_theming/templates/djust_theming/gallery/gallery.html` uses `{% load theme_components %}` to render every component via its template tag. URL included from `djust_theming/urls.py` via `path("gallery/", include("djust_theming.gallery.urls"))`. 17 new tests covering URL resolution, access control (DEBUG/staff/anonymous), content assertions for all 24 component headings, variant/size rendering, preset switcher form, preset query parameter, and context builder completeness.
